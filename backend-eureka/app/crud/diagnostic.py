...
def _compute_stats(diagnostic: Diagnostic) -> DiagnosticOut:
    ...
    # Inyectar número de trabajadores de la empresa para pre-poblar el form
    if hasattr(diagnostic, "company") and diagnostic.company:
        out.company_num_trabajadores = diagnostic.company.num_trabajadores
    return out
...
def _load_diagnostic(db: Session, diagnostic_id: int) -> Optional[Diagnostic]:
    stmt = (
        select(Diagnostic)
        .options(
            selectinload(Diagnostic.answers),
            selectinload(Diagnostic.company),
        )
